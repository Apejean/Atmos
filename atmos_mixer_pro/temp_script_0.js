    ...
    function buildRoom(w, d, h) {
      ...
      const divisions = Math.max(4, Math.round(maxDim));
      const gridHelper = new THREE.GridHelper(maxDim, divisions, 0x38bdf8, 0x1e293b);
      gridHelper.position.set(0, 0, 0);
      roomGroup.add(gridHelper);

      // 4.2 Room Bounding Wireframe Box
      const boxGeo = new THREE.BoxGeometry(w, h, d);
      const edgesGeo = new THREE.EdgesGeometry(boxGeo);
      const lineMat = new THREE.LineBasicMaterial({ color: 0x0284c7, transparent: true, opacity: 0.6, linewidth: 2 });
      const wireframe = new THREE.LineSegments(edgesGeo, lineMat);
      wireframe.position.set(0, h / 2, 0);
      roomGroup.add(wireframe);

      // 4.3 Floor Perimeter Neon Line
      const floorPts = [
      ...
      const floorMat = new THREE.LineBasicMaterial({ color: 0x38bdf8, linewidth: 2 });
