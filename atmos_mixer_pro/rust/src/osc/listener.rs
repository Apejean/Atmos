            ...
            let (theme_start_info, is_system_reset) = {
                ...
                if let Some(config) = config_guard.as_ref() {
                    ...
                    let is_sr = !config.system_reset_osc_address.is_empty()
                        && msg.addr == config.system_reset_osc_address;

                    if is_ts {
                        if let Some(first_room) = config.rooms.first() {
                            let track_ids: Vec<String> = first_room
                                .tracks
                                .iter()
                                .filter(|t| t.is_loop)
                                .map(|t| t.id.clone())
                        ...
                        if let Some(ref active_id) = *active {
                            if active_id != &room.id {
                                crate::core::state::GLOBAL_STATE.log(format!(
                                    "Drop OSC (Clear): {} is not the active room",
                                    room.id
                                ));
                                continue;
                            }
                                        ...
                                        if let Some(data) = data_opt {
                                            ...
                                            if !is_playing {
                                                let instance_id = std::time::SystemTime::now()
                                                    ...
                                                    .as_nanos()
                                                    as u64;
                                                crate::core::state::GLOBAL_STATE.add_playing_track(
                                                    instance_id,
                                                    next_t.id.clone(),
                                                );
                    ...
                    if track.play_osc_address == msg.addr {
                        // Check Gating
                        {
                            ...
                            if let Some(ref active_id) = *active {
                                if active_id != &room.id {
                                    crate::core::state::GLOBAL_STATE.log(format!(
                                        "Drop OSC (Play): {} is not the active room",
                                        room.id
                                    ));
                                    continue;
                                }
                        ...
                        if let Some(data) = data_opt {
                            ...
                            crate::core::state::GLOBAL_STATE
                                .log(format!("Cache miss for track file: {}", track.file_path));
                        }
                        matched = true;
                    }
                    if track.stop_osc_address == msg.addr {
                        // Check Gating
                        {
                            ...
                            if let Some(ref active_id) = *active {
                                if active_id != &room.id {
                                    crate::core::state::GLOBAL_STATE.log(format!(
                                        "Drop OSC (Stop): {} is not the active room",
                                        room.id
                                    ));
                                    continue;
                                }
