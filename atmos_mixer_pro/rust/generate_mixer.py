...
content = """use crate::audio::player::SoundInstance;
    ...
    pub fn process(&mut self, output: &mut [f32], out_channels: usize) {
        ...
        for frame in 0..frames {
            ...
            for (i, instance_opt) in self.instances.iter_mut().enumerate() {
                ...
                let mut vals = [0.0; 16]; // N-channel temp buffer (up to 16 channels)
                let ch_limit = channels.min(16);
...
with open("src/audio/mixer.rs", "w") as f:
    f.write(content)
