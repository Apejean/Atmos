...
process_end = content.find("pub fn update_hrtf", process_start)

if process_end == -1:
    print("Could not find update_hrtf")
else:
    # Need to skip back over any whitespace/newlines before update_hrtf
    while content[process_end-1].isspace():
        process_end -= 1
    content = content[:process_start] + process_code + "\n\n    " + content[process_end:]

# Update VirtualMixRoomBinaural::new block_size to 8192
content = content.replace("Binaural::new(channels, 1024)", "Binaural::new(channels, 8192)")
